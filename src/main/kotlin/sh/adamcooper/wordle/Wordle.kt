...
class Wordle {
    ...
    val bestWord: String by lazy {
        ...
        this.findBestWords(this.wordList, n = 1, uniqueOnly = true).asSequence().first().key.also {
            Wordle.log.info("Found best word $it")
        }
    }

    /**
     * Finds the best word in the word list by counting the most common characters and then scoring
     * all the words with unique characters based on those counts.
     */
    ...
    fun findBestWords(
            ...
            words
                ...
                .associateWith { word ->
                    word.sumOf(counter::getValue) +
                        if (word.toSet().size == word.length) 1000 else 0
                }
        ...
        if (tieBreaker.count() > 1) {
            // Calculate new scores based on how often each letter in the word will make a green
            // square
            val greenScores =
                tieBreaker
                    .map { it.key }
                    .associateWith { tiedWord ->
                        // Take the sum of all the times a letter in the tied word shows up in the
                        // same
        ...
        return topWords +
            ...
            sortedWords
                .asSequence()
                .filterNot { topWords.contains(it.key) }
                ...
                words
                    .filter { word ->
                        ...
                        word.withIndex().all { (i, c) ->
                            ...
                            green.asSequence().all { (c, indices) -> indices.all { word[it] == c } }
                    }
                    .toMutableList()
    ...
    companion object {
        ...
        private const val WORD_LIST_URL =
